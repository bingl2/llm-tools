...
시간을 관리하는 것과 별개로, 우리는 하루에도 수십 번 무언가를 느끼고, 떠올리고, 깨닫습니다. 코드 리뷰에서 답답함을 느꼈을 때, 누군가의 블로그 글에서 영감을 받았을 때, 퇴근길에 문득 든 생각 — 이런 것들은 할 일 목록에 들어가지 않아요. 그래서 휘발됩니다.

`/timebox-think`는 이 **생각의 조각들을 포착하는 도구**예요. 마구잡이로 쏟아내면, 원문은 한 글자도 건드리지 않고 보존하면서 AI가 정제하고 코칭 피드백을 붙여줍니다. Foundation에 적힌 당신의 가치관과 목표를 거울처럼 비추어, 감정에 매몰되지 않되 감정을 부정하지도 않는 피드백이에요.

핵심은 세 가지:
- **즉시성** — 생각은 휘발됩니다. 떠오른 순간 바로 캡처.
- **원문 보존** — 날것의 기록이 가장 가치 있어요. 오탈자도, 비문도 그대로.
- **자산화** — 옵시디언에 쌓이는 think 기록은 나중에 글쓰기, 회고, 의사결정의 원재료가 됩니다.

timebox가 "시간"을 설계하는 도구라면, think는 **"생각"을 설계하는 도구**예요. 둘이 합쳐지면 "뭘 했는지"뿐 아니라 "뭘 느꼈는지"까지 기록으로 남는 하루가 됩니다.
...
- `/timebox-think`가 순간의 생각/감정을 캡처하여 Foundation 기반 코칭과 함께 옵시디언 노트로 저장
...
순간적으로 떠오르는 생각, 감정, 영감, 관찰을 즉시 캡처. 업무든, 미디어 감상이든, 철학적 고민이든 주제를 가리지 않아요.

**흐름:** Foundation 로드 → 브레인덤프 수용 → 분석 + 노트 생성 → 파일 저장 → (옵시디언 자동 열기)

- **원문 100% 보존**: 사용자가 쏟아낸 날것의 텍스트는 한 글자도 수정하지 않음
- **Foundation 기반 코칭**: 뻔한 위로 대신, 사용자의 가치관과 목표를 거울처럼 비추는 피드백
- **Obsidian 연동**: `obsidian_vault: on`이면 노트 자동 열기 + 데일리 노트에 백링크 추가
- **독립 동작**: timebox plan 없이도 동작. 언제든 자유롭게 사용

**출력:** `thinks/{YYYY-MM-DD}/{YYYYMMDD}-{HHMM}-{short_name}.md`

```markdown
---
date: 2026-03-28 14:30
tags: [think-log, code-review, communication]
mood: 😤frustrated
---
...
├── thinks/
│   └── {YYYY-MM-DD}/
│       └── {YYYYMMDD}-{HHMM}-{short_name}.md  # 생각 캡처 (/timebox-think)
└── reviews/
...
| Obsidian 연동 | off | `_config.md` (`/timebox-setup`에서 설정) |
